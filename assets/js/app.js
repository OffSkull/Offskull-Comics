...
function renderComic() {
  ...
  const requestedComicId = qs("id");

  /*
   * Если ID комикса не указан,
   * показываем каталог всех комиксов.
   */

  if (!requestedComicId) {
    renderComicCatalog(container);
    return;
  }

  /*
   * Ищем выбранный комикс.
   */

  const comic =
    data.comics.find(
      item => item.id === requestedComicId
    );
      ...
      <section class="section">
        ...
        <div class="empty-state">
          ...
          <p>
            ...
            или ссылка указана неправильно.
          </p>
...
function renderComicCatalog(container) {
  const comics =
  ...
  container.innerHTML = `
    <section class="page-hero">
      ...
      <div class="narrow">
        ...
        <p>
          Выберите комикс, чтобы посмотреть
